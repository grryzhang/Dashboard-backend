...
@Aspect
@Component
public class ControllerExceptionAOPHandler {
	
	private static Logger log = Logger.getLogger(ControllerExceptionAOPHandler.class);  
	
	@Autowired
	private SysErrorLogDAO sysErrorLogDAO;

	@Pointcut(value="execution(* com.zhongzhou.Excavator.web.controller..*.*(..))")
	public void interceptorException(){
		
	}
	
	@Around(value="interceptorException()")  
	public Object logException( final ProceedingJoinPoint pjp ){
		...
		try{
			...
			try{
				...
				errorLog.setExceptionId( e.getClass().getName() + "@" + e.hashCode() );
